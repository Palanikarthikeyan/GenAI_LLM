...
def create_vector_embedding():
    if "vectors" not in st.session_state:
        st.session_state["vectors"] = []  # or any appropriate initial value
...
if user_prompt:
    ...
    with st.expander("PDF similarity search:"):
        for v1,v2 in enumerate(response["context"]):
            st.write(v2.page_content)
            st.write("---------------")